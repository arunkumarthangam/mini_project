<?php
include('../../db.php');
$result=mysqli_fetch_row(mysqli_query($con,"Select fee,lfee,one_time,late_date from sch_cls_fee where Medium='".trim($_POST['fee_chng_mdm'])."' AND Std='".trim($_POST['fee_chng_std'])."' And fee_type='".trim($_POST['fee_chng_type'])."'"));
if($result){
	//print_r($result[2]);
	if($result[2]=='Per Year'){
		...
		$q1=mysqli_query($con,"select Month from sch_tran where Gr_num='".$_POST['fee_chng_grn']."' AND fee_type='".$_POST['fee_chng_type']."'");
